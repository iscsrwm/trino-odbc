...
static trino_desc_record_t *trino_desc_get_record(trino_descriptor_t *desc, SQLUSMALLINT rec)
{
                               ...
                               SQLINTEGER field, SQLPOINTER value, SQLINTEGER str_len)
{
    ...
    if (field == SQL_DESC_COUNT) {
        ...
        if (new_count > desc->alloc_count) {
            ...
            trino_desc_record_t *new_records = realloc(desc->records,
                new_alloc * sizeof(trino_desc_record_t));
            if (!new_records) return SQL_ERROR;
    ...
    switch (field) {
        ...
        case SQL_DESC_PRECISION:
            rec->decimal_digits = *(SQLSMALLINT *)value;
            break;
        case SQL_DESC_SCALE:
            rec->decimal_digits = *(SQLSMALLINT *)value;
            break;
        case SQL_DESC_NULLABLE:
            rec->nullable = *(SQLSMALLINT *)value;
            break;
        ...
        case SQL_DESC_INDICATOR_PTR:
            rec->str_len_or_ind = (SQLLEN *)value;
            break;
        default:
            return SQL_SUCCESS; /* silently ignore unknown fields */
    }
    return SQL_SUCCESS;
}
    ...
    switch (field) {
        ...
        case SQL_DESC_NAME:
            if (value && buffer_length > 0) {
                strncpy((char *)value, (char *)rec->column_name, (size_t)buffer_length - 1);
