...
char *trino_wchars_to_utf8(const SQLWCHAR *wstr, size_t wlen)
{
    ...
    int required_len = WideCharToMultiByte(CP_UTF8, 0, (LPCWCH)wstr, (int)wlen,
                                           NULL, 0, NULL, NULL);
    if (required_len <= 0) {
        return NULL;
    }
    ...
    WideCharToMultiByte(CP_UTF8, 0, (LPCWCH)wstr, (int)wlen,
                        result, required_len, NULL, NULL);
    ...
    for (i = 0; i < wlen && p[i] != 0; i++) {
        ...
        if (ch >= 0xD800 && ch <= 0xDBFF && i + 1 < wlen && 
            p[i+1] >= 0xDC00 && p[i+1] <= 0xDFFF) {
...
SQLWCHAR *trino_utf8_to_wchars(const char *str, size_t *out_wlen)
{
    ...
    int required_len = MultiByteToWideChar(CP_UTF8, 0, str, (int)len,
                                           NULL, 0);
    if (required_len <= 0) {
        return NULL;
    }
