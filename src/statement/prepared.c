...
static int format_parameter_value(SQLSMALLINT C_type, SQLPOINTER value, char *out,
                                  size_t out_size)
{
    if (!value || !out || out_size < 8)
        return -1;

    switch (C_type) {
        case SQL_C_CHAR: {
            /* String - wrap in single quotes and double any embedded quote. */
            if (strlen((char *)value) * 2 + 3 >= out_size)
                return -1;
            size_t pos = 0;
            out[pos++] = '\'';
        ...
        case SQL_C_UTINYINT:
            ...
            break;
        case SQL_C_SSHORT:
        case SQL_C_SHORT: snprintf(out, out_size, "%d", (int)*(short *)value); break;
        case SQL_C_USHORT:
        ...
        case SQL_C_UBIGINT:
            ...
            break;
        case SQL_C_FLOAT: snprintf(out, out_size, "%.9g", (double)*(float *)value); break;
        case SQL_C_DOUBLE: snprintf(out, out_size, "%.17g", *(double *)value); break;
        case SQL_C_BIT:
                           ...
                           SQLLEN buffer_length, SQLLEN *str_len_or_ind)
{
    ...
    return trino_stmt_bind_param(stmt, parameter_number, parameter_type, C_type,
                                 column_size, decimal_digits, parameter_value_ptr,
                                 str_len_or_ind);
}
                                ...
                                SQLPOINTER parameter_value, SQLLEN *str_len_or_ind)
{
    ...
    stmt->param_count =
        param_number > stmt->param_count ? param_number : stmt->param_count;
...
{
    if (!statement_handle || !parameter_count_ptr)
        return SQL_INVALID_HANDLE;
...
static int append_bytes(char **buf, size_t *cap, size_t *used, const char *src,
                        size_t len)
{
    if (*used + len + 1 > *cap) {
        size_t newcap = (*cap ? *cap : 256);
        while (*used + len + 1 > newcap)
            newcap *= 2;
...
static char *substitute_parameters(trino_stmt_t *stmt, const char *sql, int param_count)
{
    if (!sql)
        return NULL;
    ...
    while (*src) {
        if (in_string) {
            if (append_bytes(&result, &cap, &used, src, 1) != 0)
                goto fail;
            if (*src == string_char) {
        ...
        if (*src == '?') {
            ...
            if (param_index < (int)stmt->ipd->alloc_count) {
                ...
                bool is_null =
                    (rec->str_len_or_ind && *rec->str_len_or_ind == SQL_NULL_DATA);
                if (rec->data_ptr && !is_null &&
                    format_parameter_value(rec->c_type, rec->data_ptr, buffer,
                                           sizeof(buffer)) == 0) {
                    literal = buffer;
            ...
            if (append_bytes(&result, &cap, &used, literal, litlen) != 0)
                goto fail;
            param_index++;
            src++;
            continue;
