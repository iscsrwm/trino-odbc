...
{
    snprintf(out, out_size, "Server=%s;Port=%s;User=%s;Catalog=%s;Schema=%s;SSL=%s",
             env_or("TRINO_TEST_SERVER", "localhost"), env_or("TRINO_TEST_PORT", "8080"),
             env_or("TRINO_TEST_USER", "test"), env_or("TRINO_TEST_CATALOG", "tpch"),
             env_or("TRINO_TEST_SCHEMA", "tiny"), env_or("TRINO_TEST_SSL", "false"));
}
...
static SQLRETURN connect_live(SQLHENV *env, SQLHDBC *dbc)
{
    ...
    return SQLDriverConnect(*dbc, NULL, (SQLCHAR *)conn_str, SQL_NTS, out, sizeof(out),
...
static void test_select_rows(void)
{
    ...
    CHECK(SQLExecDirect(stmt,
                        (SQLCHAR *)"SELECT name FROM tpch.tiny.nation ORDER BY name",
                        SQL_NTS) == SQL_SUCCESS);
    ...
    while (SQLFetch(stmt) == SQL_SUCCESS) {
        ...
        if (SQLGetData(stmt, 1, SQL_C_CHAR, buf, sizeof(buf), &ind) == SQL_SUCCESS) {
            if (count == 0)
                strncpy(first, buf, sizeof(first) - 1);
            count++;
        }
    }
...
static void test_bound_parameter(void)
{
    ...
    SQLINTEGER param = 7;
    SQLLEN plen = 0;
    CHECK(SQLBindParameter(stmt, 1, SQL_PARAM_INPUT, SQL_C_LONG, SQL_INTEGER, 0, 0,
                           &param, 0, &plen) == SQL_SUCCESS);

    CHECK(SQLExecDirect(
              stmt, (SQLCHAR *)"SELECT name FROM tpch.tiny.nation WHERE nationkey = ?",
              SQL_NTS) == SQL_SUCCESS);
...
static void test_query_error(void)
{
    ...
    CHECK(SQLExecDirect(stmt, (SQLCHAR *)"SELECT * FROM tpch.tiny.no_such_table",
                        SQL_NTS) == SQL_ERROR);
